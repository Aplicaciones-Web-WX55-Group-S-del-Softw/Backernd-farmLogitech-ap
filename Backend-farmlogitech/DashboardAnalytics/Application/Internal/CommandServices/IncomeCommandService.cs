...
public class IncomeCommandService : IIncomeCommandService
{
   private readonly IIncomeRepository _incomeRepository;
    ...
    public IncomeCommandService(IIncomeRepository incomeRepository, IUnitOfWork unitOfWork, IUserRepository userRepository, IFarmRepository farmRepository)
    {
    ...
    public async Task<Income> Handle(CreateIncomeCommand command)
    {
        ...
        {
            throw new Exception("Only users with role FARMER or FARMWORKER can create a Income");
        }
        ...
        var existingIncome = await _incomeRepository.GetByCategoryAndDate(command.Category, command.Date);
        if (existingIncome != null)
        {
            throw new Exception("Income with Category and Date already exists");
        }

        // Crea un nuevo cultivo con el comando proporcionado y asigna el FarmId 
        var incomeNew = new Income(command)
        {
            FarmId = farmId,
        };

        // Agrega el nuevo cultivo al repositorio
        await _incomeRepository.AddAsync(incomeNew);
